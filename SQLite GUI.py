...
#conn = sqlite3.connect('MotionAnalysis16.db')
c=conn.cursor()
...
def browseFiles1(): 
    global filename1
    filename1 = ntpath.basename(filedialog.askopenfilename(initialdir = "C:/Users/johan/Documents/MEng FYP/C3D_files", 
                                          title = "Select a File", 
...
def enter_patient_data():
    c.execute("""INSERT INTO Patient_Data
              ...
              VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
              (ID, DomLimb, pattype, month, AffectedLimb, filename1, filename2, filename3, filename4, filename5, filename6))
numTrials = []
def enter_Gait_Table (numTrialslist):
    if v1c.get() == 1:
        numTrials.append("Trial 1")
    if v2c.get() == 1:
        numTrials.append("Trial 2")
    if v3c.get() == 1:
        numTrials.append("Trial 3")
    if v4c.get() == 1:
        numTrials.append("Trial 4")
    if v5c.get() == 1:
        numTrials.append("Trial 5")
    if v6c.get() == 1:
        numTrials.append("Trial 6")
    for TrialNum in numTrialslist:
        if TrialNum=="Trial 1":
            filename = filename1
...
buttonaccept = Button(FrameClose, text = "Accept", 
                      command  = lambda: [enter_patient_data(), enter_Gait_Table(numTrials), uncheck(), close_window()])
